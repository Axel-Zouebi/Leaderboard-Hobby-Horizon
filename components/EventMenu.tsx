...
export function EventMenu() {
    ...
    return (
        ...
        >
            {/* Hamburger Button */}
            <button
                className="p-2 rounded-lg hover:bg-slate-200 transition-colors flex items-center gap-2"
            ...
            {/* Dropdown Menu */}
            {isHovered && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 overflow-hidden z-50">
                    {EVENTS.map((event) => (
                        <button
                            key={event.id}
                            onClick={() => handleEventChange(event.id)}
                            className={`w-full px-4 py-3 text-left hover:bg-slate-50 transition-colors flex items-center justify-between ${
                                currentEvent === event.id ? 'bg-slate-100 font-semibold' : ''
                            }`}
                        >
